...
async function serveComponent() {
  ...
  const router = `
    ...
    document.title = '${target} | Arsenal Design'
        ...
        ${demos
          .map((demo, index) => {
            return `{
              path: '${index ? `/${demo}` : '/'}',
              name: '${demo}',
              component: () => import('../../docs/demos/${target}/${demo}/index.vue')
            }`
          })
          .join(',\n')},
          {
    ...
    router.afterEach(to => {
      document.title = \`${target} - \${typeof to.name === 'string' ? to.name : 'dev'} | Arsenal Design\`
    })
  `

  writeFileSync(
    resolve(devDir, 'router', `port-${port}.ts`),
    await prettier.format(router, { ...prettierConfig, parser: 'typescript' }),
    'utf-8',
  )
