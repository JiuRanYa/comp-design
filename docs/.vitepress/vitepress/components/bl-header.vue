...
import { Moon, Sun } from '@arsenal-design/icons'
...
const components = computed(() => {
  return siderbarsConfig.components
    .reduce((res, item) => {
    ...
    <div class="bl-header">
      <div class="bl-header-left">
        <a class="bl-logo" href="/">
          <span class="bl-title">Arsenal Design</span>
        </a>
        <div class="bl-nav">
          <a
            v-for="item in nav"
            :key="item.text"
            :href="item.link" class="link"
            :class="{
      ...
      <div class="bl-header-right">
        <Dropdown v-model:visible="visible" placement="bottom-start">
          <div class="bl-header-search">
            <input ref="inputRef" v-model="inputSearchKey" placeholder="在Arsenal Design中搜索">
            <kbd>⌘ K</kbd>
          </div>
          ...
          <template #drop>
            <DropdownList style="width: 220px">
              <ScrollArea max-height="400px">
                <DropdownItem v-for="comp in components" :key="comp.text">
                  <a
                    style="display: inline-block; width: 100%"
                    :href="getProjectLink('arsenal-design', comp.link)"
